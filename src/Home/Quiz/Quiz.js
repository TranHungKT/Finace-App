...
class Quiz extends Component {
  ...
  loadQuestion = () => {
    this.setState(() => {
      return {
      question: this.props.user.questions[this.state.currentQuestion].question,
  ...
  render() {
    ...
    let checkQuestion = options.map((option,index)=> (
      this.state.myAnswer !== option ? (
        <View key = {index} >
        <Card>
              <CardItem button bordered = {true} borderedColor = {'#EBEBEB'} onPress = {() => this.answerQuestion(option)}>
                <Body>
                  <Text
        ...
        <View key = {index}>
        <Card>
              <CardItem button bordered = {true} borderedColor = {'#EBEBEB'} onPress = {(option) => this.answerQuestion(option)}
    ...
    let submitButton = this.props.user.questions[currentQuestion].id + 1 == this.props.user.questions.length  ? (
      <Button 
    ...
    return (
        <View style = {{flex:1 ,marginHorizontal: 20}}>
            <Text style = {styles.text}>QUESTION {this.props.user.questions[currentQuestion].id + 1} OF {this.props.user.questions.length}</Text>
